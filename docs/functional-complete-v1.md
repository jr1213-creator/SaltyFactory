...
| Publish Review | Provider-backed draft actions | `/studio/publish-review` computes gates from persisted evidence, shows blockers/provider readiness, exposes real "Send to Printify" and "Create Shopify Draft" actions, and displays persisted provider proof after success: Printify upload/product IDs plus Shopify draft/media/collection evidence. Approval alone does not publish or sync. |
