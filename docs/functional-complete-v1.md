...
## Tactile Coastal Studio Design Pass

Studio's first-viewport shell now uses the tactile coastal-brutalist design system:

- exact SaltyFactory `--sf-*` palette tokens
- CSS-generated topo, sand, canvas, denim, grid-paper, and stitch texture utilities
- global command navigation with Home, POD Factory, AI Workforce, Business, Storefront, Marketing, Customers, and Operations
- keyboard-aware mega menus and mobile accordion coverage
- contextual workflow sidebar instead of an always-expanded route dump
- reusable bento primitives in `packages/ui`
- key POD route labels updated to owner workflow language: `Generated art gallery`, `Mockup gallery`, `One-product build flow`, `Choose the product shell`, and `Launch gate control`

This design pass is presentation-only for runtime behavior. It does not weaken auth, expose provider secrets, fake provider success, bypass owner approval, change private storage, or enable live publish.
