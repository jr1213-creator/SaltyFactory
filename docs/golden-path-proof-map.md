...
- Live Printify mockup smoke exists as `corepack pnpm smoke:printify-mockups-live` and is skipped unless `RUN_LIVE_PRINTIFY_MOCKUP_SMOKE=true` plus `PRINTIFY_SMOKE_CONFIRMATION="CREATE TEST PRINTIFY PRODUCT"`. It was added but not run in this session because it would create a real test product in Printify.
