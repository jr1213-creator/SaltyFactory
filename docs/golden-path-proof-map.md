...
| Printify mockup workflow | `/studio/mockups` | `/api/studio/integrations/printify/uploads`, `/api/studio/integrations/printify/products/create`, `/api/studio/integrations/printify/mockups/import` | Printify upload ID, Printify product ref, `print_areas`, and `mockup_assets` rows sourced from Printify product `images` | Product Builder, Publish Review, Shopify media handoff | `tests/printify-runtime-resolver.test.ts`, `tests/pod-golden-path-execution.test.ts` |
| Printify mockup preview | `/studio/mockups`, product/publish pages | Printify provider image URL stored on `mockup_assets` metadata | Provider mockup URL plus source asset/draft/product proof; no private token or storage path in owner UI | Product Builder, Publish Review, Shopify draft route | `tests/pod-golden-path-execution.test.ts` |
| Product draft | `/studio/product-builder` | `/api/studio/drafts/create-from-assets` | Product draft with asset ID, real Printify mockup IDs, pricing, collection, tags, provider target | Printify catalog selection and Publish Review | `tests/pod-golden-path-execution.test.ts` |
...
- Live image/internal-renderer provider smoke is passing locally. On 2026-07-04, `RUN_LIVE_IMAGE_MOCKUP_SMOKE=true corepack pnpm smoke:image-mockup-live` created brief `brief_1783178628635`, generated Hugging Face asset `asset_hf_1783178630554_0_cdd6aebf`, created derivatives `thumbnail`, `web_preview`, and `print_png`, rendered internal proof mockup `mockup_1783178637936_tmpl_internal_apparel_light_tee_55d058`, verified protected previews, and printed no provider/storage secrets.
- Focused authenticated browser proof exists as `corepack pnpm frontend:qa:image-mockup`. On 2026-07-04 it passed with the guarded local proof provider. After the Printify pivot, `/studio/mockups` shows the Printify product/mockup workflow and does not present internal templates as production mockups.
...
- Live Printify mockup smoke passed on 2026-07-04 with `RUN_LIVE_PRINTIFY_MOCKUP_SMOKE=true PRINTIFY_SMOKE_CONFIRMATION="CREATE TEST PRINTIFY PRODUCT" corepack pnpm smoke:printify-mockups-live`. It used guided credential-store credentials, source asset `asset_hf_1783177574961_0_4f15cb21`, `print_png`, upload ID `6a495803f2b0395d7831a573`, product ID `6a495f94ea82e6b84903c9e6`, and imported `3` Printify product images. The hero/default mockup row is `mockup_printify_6a495f94ea82e6b84903c9e6_1783193496107_0`. Shopify publish and live sync were not called.
- Worker queued generation currently persists one generated output plus derivatives. Browser route supports up to four variants; worker multi-variant support remains partial. The compatibility `DatabaseBackedQueue` class is in-memory/dev-only; durable worker jobs use the repository job table, and unknown worker job types now fail instead of being marked complete.
- Background removal and upscaling are explicit disabled local-worker provider gaps. No hosted AI provider has been added. Future implementation must store original-to-processed provenance and verify the deployed model artifact license, including ONNX exports; BRIA RMBG must not be used for commercial POD output unless a commercial license is explicitly configured.
- Shopify currently uses the REST Admin adapter in the repo. GraphQL Admin API migration is a future plan for product/collection mutations, not required for the current verified draft path. Smart collections are rule-managed and are not valid manual assignment targets.
