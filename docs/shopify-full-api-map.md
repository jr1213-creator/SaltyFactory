...
| Draft product creation | `POST /api/studio/publish/shopify`, `ShopifyAdminProviderLive.createProductDraft` | `/studio/publish-review` | Creates Shopify products with `status: draft` only. No live publish. |
...
| Collection assignment | `ShopifyAdminProviderLive.assignCollection` | `/studio/publish-review` optional collection ID | Creates a Shopify collect when a collection ID is provided and configured. |
...
It also requires persisted product draft, publish review, passing gates, product variants/pricing, approved mockup media, and owner provider permission.
