...
| --- | --- | --- | --- | --- |
| A | Replace Studio nav with two-tier POD sidebar + top dropdowns | Partial | `StudioTopNavDropdowns` now adds top command-center dropdowns fed by `STUDIO_NAV_SECTIONS`, while the grouped sidebar remains as the second-tier navigation. | Full POD-only sidebar IA simplification and authenticated browser crawl still remain. |
