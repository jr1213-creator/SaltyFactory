...
export function getIntegrationStates(config: RuntimeConfig): IntegrationState[] {
  return [
    ...
    { key: "google_oauth", label: "Google OAuth", status: config.GOOGLE_CLIENT_ID && config.GOOGLE_CLIENT_SECRET ? "configured" : "missing_credentials", capabilities: ["oauth_authorization", "token_refresh"], setupRequired: config.GOOGLE_CLIENT_ID && config.GOOGLE_CLIENT_SECRET ? [] : ["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET"], scopes: [...googleScopes.ga4, ...googleScopes.searchConsole] },
    ...
    { key: "supabase_storage", label: "Supabase Storage", status: config.SUPABASE_URL && config.SUPABASE_SERVICE_ROLE_KEY ? "configured" : "missing_credentials", capabilities: ["private_assets", "public_approved_assets"], setupRequired: config.SUPABASE_URL && config.SUPABASE_SERVICE_ROLE_KEY ? [] : ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"] },
    { key: "hugging_face", label: "Hugging Face", status: config.providers.aiText.enabled || config.providers.aiImage.enabled ? "configured" : "disabled", capabilities: ["draft_text_generation", "draft_image_generation"], setupRequired: config.providers.aiText.enabled || config.providers.aiImage.enabled ? [] : ["AI_TEXT_ENABLED or AI_IMAGE_ENABLED", "HF_API_TOKEN", "model env vars"] },
