...
const shopifySetupRequired = (config: RuntimeConfig) =>
  setupIfMissing(config.providers.shopifyAdmin.enabled, [
    "SHOPIFY_ADMIN_ENABLED=true",
    "SHOPIFY_STORE_DOMAIN",
    "SHOPIFY_CLIENT_ID and SHOPIFY_CLIENT_SECRET, or legacy SHOPIFY_ADMIN_TOKEN if available"
  ]);
...
export function getIntegrationStates(config: RuntimeConfig): IntegrationState[] {
  return [
    ...
    { key: "shopify", label: "Shopify", status: stateFromProvider(config.providers.shopifyAdmin.enabled, config.providers.shopifyAdmin.reason, ["SHOPIFY_ADMIN_ENABLED", "SHOPIFY_STORE_DOMAIN", "SHOPIFY_CLIENT_ID", "SHOPIFY_CLIENT_SECRET"]), capabilities: ["catalog_import", "draft_export_guarded", "webhook_verification"], setupRequired: shopifySetupRequired(config) },
