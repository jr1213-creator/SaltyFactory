...
export class DrizzleProductDraftRepository extends DrizzleBaseRepository { constructor(db?: DbClient, audit?: AuditWriter) { super("product_drafts", db, audit); } async listApprovedForStorefront(workspaceId: string) { return (await this.listByWorkspace(workspaceId)).filter((row) => row.status === "published" || row.approval_status === "approved" || row.approvalStatus === "approved").map(toPublicStorefrontProjection); } }
