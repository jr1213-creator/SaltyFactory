...
export const setupFieldGuides: SetupFieldGuide[] = [
  ...
  guide({
    ...
    plainEnglishDescription: "A private Admin API token that lets SaltyFactory create Shopify draft products and upload approved media.",
    whyNeeded: "Shopify requires an Admin API token for draft product creation, media upload, and collection assignment.",
    whereToGetIt: "Create a custom app in Shopify Admin and install it to reveal the Admin API access token.",
    providerUrl: "https://admin.shopify.com/",
    stepsToFindIt: ["Open Shopify Admin.", "Open Settings, then Apps and sales channels.", "Choose Develop apps or create a custom app.", "Grant product and collection permissions.", "Install the app and copy the Admin API access token."],
    recommendedScopes: ["read_products", "write_products", "read_publications", "read_content"],
    securityNote: "The Admin token is write-only. It is encrypted server-side when credential storage is enabled and is never returned to the browser.",
    validationAction: "/api/studio/provider-connections/shopify/validate-admin",
    commonProblems: ["Store domain and token do not belong to the same store.", "Token lacks product write permission.", "Custom app was not installed."],
    troubleshootingSteps: ["Reinstall the custom app.", "Confirm write_products permission.", "Validate again before creating drafts."],
    setupMode: "secure_token",
    sensitivity: "secret",
    showInOwnerSetup: true,
    showInAdvancedOnly: false,
  ...
  guide({
    ...
    troubleshootingSteps: ["Create the collection in Shopify.", "Validate the Admin token again."],
    setupMode: "manual",
    sensitivity: "public",
    showInOwnerSetup: true,
    showInAdvancedOnly: false,
...
export function buildOwnerSetupCards(report: FeatureReadinessReport): OwnerSetupCard[] {
  ...
  return [
    ...
    createCard({
      ...
      validateHref: "/api/studio/provider-connections/shopify/validate-admin",
      advancedSummary: "Server deployment can use protected Shopify Admin env config, but owner setup should save the token securely when credential storage is enabled.",
      deploymentNotes: ["Draft creation does not publish.", "Live storefront publish requires separate owner confirmation and live publish flags."]
    }),
    createCard({
      feature: storage,
      providerKey: "storage",
