...
export default async function Page({ searchParams }: { searchParams?: Promise<{ draft_id?: string }> } = {}) {
  ...
  return <>
    <PageHeader title="Product Draft Editor" description="Prepare product content, variants, pricing, SEO, and validation before publish review.">
      <button className="btn btn-secondary" disabled title="Draft duplication is not implemented.">Duplicate Draft</button><button className="btn btn-secondary" disabled title="Inline draft editing is disabled until the save route is wired.">Save Draft</button><button className="btn btn-primary" disabled title="Use the saved draft workflow to create a publish review.">Submit for Review</button>
    </PageHeader>
    <div className="workspace-grid">
      <div className="layout-grid">
        <DraftWorkflowClient initialDrafts={drafts as any[]} initialDraftId={params?.draft_id} />
        <Card><h2>Product Information</h2><form className="form-grid"><label>Product title<input defaultValue={draft.title ?? ""} placeholder="Untitled draft" readOnly title="Inline draft editing is disabled until the save route is wired." /></label><label>Collection<select disabled title="Inline draft editing is disabled until the save route is wired."><option>{draft.collection ?? "Choose collection"}</option></select></label><label>Tags<input defaultValue={(draft.tags ?? []).join(", ")} placeholder="western, coastal, original" readOnly title="Inline draft editing is disabled until the save route is wired." /></label></form><p className="text-muted">Use the workflow panel for saved validation, review creation, and projection requests.</p></Card>
        <PriceMarginPanel />
        ...
        <Card><h2>Fulfillment & Publish Targets</h2><p className="text-muted">Publishing is not available from the draft editor. Submit to review first.</p></Card>
      </div>
      <div className="layout-grid">
        <Card><h2>Product Description</h2><textarea className="code-block" defaultValue={draft.description ?? "Draft copy awaits human review."} readOnly title="Inline draft editing is disabled until the save route is wired." /><h2>SEO</h2><StructuredDataPreview title={draft.title ?? "Product Draft"} /></Card>
        <Card><h2>AI Readiness Score</h2><ProgressRing value={76} label="Draft" /><p className="text-muted">Suggestions are local UI only until providers are configured.</p></Card>
      </div>
      <div className="layout-grid">
