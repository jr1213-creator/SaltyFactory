...
export default async function DecisionMemosPage() {
  ...
  return <>
    ...
    <section className="surface-card">
      ...
      <form className="form-grid" action="/api/studio/business/decision-memos" method="post">
        ...
        <button className="btn btn-primary" type="submit">Save Decision Memo</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{memos.length ? <div className="layout-grid">{memos.map((memo: any) => <DecisionMemoPanel key={memo.id} title={memo.title}>
      <DataTable columns={["Type", "Decision", "Open"]} rows={[[memo.decision_type ?? memo.decisionType, <ApprovalBadge key="decision" status={String(memo.owner_decision ?? memo.ownerDecision)} />, <a key="open" className="btn btn-secondary" href={`/studio/business/decision-memos/${memo.id}`}>Review</a>]]} />
