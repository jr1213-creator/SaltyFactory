...
export default async function BusinessGoalsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      <h2>Create Goal</h2>
      <form className="sf-form-grid" action="/api/studio/business/goals" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Save Goal</button>
      </form>
    </section>
    <section className="sf-card" style={{ marginTop: 18 }}>{goals.length ? <DataTable columns={["Goal", "Type", "Priority", "Status"]} rows={goals.map((goal: any) => [goal.title, goal.goal_type ?? goal.goalType, goal.priority, goal.status])} /> : <EmptyState title="No goals yet" description="Create goals to drive business next actions." />}</section>
  </>;
}
