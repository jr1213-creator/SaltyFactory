...
export default async function BusinessOpportunitiesPage() {
  ...
  return <>
    ...
    <section className="surface-card">
      ...
      <form className="form-grid" action="/api/studio/business/opportunities" method="post">
        ...
        <button className="btn btn-primary" type="submit">Save Opportunity</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{opportunities.length ? <div className="layout-grid">{opportunities.map((opportunity: any) => <OpportunityCard key={opportunity.id} title={opportunity.title} description={opportunity.summary} action={<a className="btn btn-secondary" href={`/studio/business/opportunities/${opportunity.id}`}>Review</a>}>
