...
export default async function BusinessDocumentsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      ...
      <form className="sf-form-grid" action="/api/studio/business/documents/generate" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Generate Owner Review Draft</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{documents.length ? <div className="sf-grid">{documents.map((document: any) => <BusinessDocumentCard key={document.id} title={document.title} status={String(document.status)}>
      <DataTable columns={["Type", "Sensitive", "Open"]} rows={[[document.document_type ?? document.documentType, document.requires_sensitive_data ? "authority required" : "no", <a key="open" className="sf-button sf-button-secondary" href={`/studio/business/documents/${document.id}`}>Review</a>]]} />
