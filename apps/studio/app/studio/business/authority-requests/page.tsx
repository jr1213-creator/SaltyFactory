...
export default async function BusinessAuthorityRequestsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      ...
      <form className="sf-form-grid" action="/api/studio/business/authority-requests" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Create Authority Request</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{requests.length ? <div className="sf-grid">{requests.map((request: any) => <AuthorityRequestPanel key={request.id} title={request.authority_type ?? request.authorityType} status={String(request.status)}>
      <DataTable columns={["Fields", "Risk", "Expires", "Actions"]} rows={[[Array.isArray(request.fields_requested) ? request.fields_requested.join(", ") : "", request.risk_level ?? request.riskLevel, request.expires_at ?? request.expiresAt ?? "not approved", <span key="actions" className="sf-action-bar"><form action={`/api/studio/business/authority-requests/${request.id}/approve`} method="post"><button className="sf-button sf-button-primary">Approve</button></form><form action={`/api/studio/business/authority-requests/${request.id}/reject`} method="post"><button className="sf-button sf-button-danger">Reject</button></form><form action={`/api/studio/business/authority-requests/${request.id}/expire`} method="post"><button className="sf-button sf-button-secondary">Expire</button></form></span>]]} />
