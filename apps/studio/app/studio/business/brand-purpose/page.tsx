...
export default async function BusinessBrandPurposePage() {
  ...
  return <>
    ...
    <section className="sf-card" style={{ marginTop: 18 }}>
      <h2>Add Mantra</h2>
      <form className="sf-form-grid" action="/api/studio/business/mantras" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Save Mantra</button>
      </form>
    </section>
    <section className="sf-card" style={{ marginTop: 18 }}>{mantras.length ? <DataTable columns={["Mantra", "Category", "Active"]} rows={mantras.map((row: any) => [row.mantra, row.category, row.active === false ? "No" : "Yes"])} /> : <EmptyState title="No mantras yet" description="Saved mantras appear on the Business Command Center." />}</section>
  </>;
}
