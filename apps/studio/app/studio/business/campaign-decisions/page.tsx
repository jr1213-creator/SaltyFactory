...
export default async function BusinessCampaignDecisionsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      ...
      <form className="sf-form-grid" action="/api/studio/business/channel-readiness/calculate" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Calculate Readiness</button>
      </form>
    </section>
    <div className="sf-grid sf-grid-3" style={{ marginTop: 18 }}>{readiness.map((row: any) => <ChannelReadinessCard key={row.id} channel={row.channel} readiness={row.readiness} />)}</div>
    <section className="sf-card" style={{ marginTop: 18 }}>{campaigns.length ? <DataTable columns={["Campaign", "Goal", "Status"]} rows={campaigns.map((campaign: any) => [campaign.name, campaign.goal, campaign.status])} /> : <EmptyState title="No campaign drafts" description="Draft campaigns created from opportunities appear here." />}</section>
  </>;
}
