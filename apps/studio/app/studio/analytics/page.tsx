...
export default async function Page() {
  ...
  return <>
    <PageHeader title="Analytics & AI Employees" description="Only imported provider data is shown. Empty states mean no live Google or commerce sync has completed.">
      <button className="btn btn-secondary" disabled title="Analytics customization is disabled until real provider data is imported.">Customize</button><button className="btn btn-primary" disabled title="Analytics export is disabled until real provider data is imported.">Export Report</button>
    </PageHeader>
    ...
    <div className="layout-rail" style={{ marginTop: 18 }}>
      <div className="layout-grid">
        <ChartCard title="Imported Google metrics">{hasGoogleMetrics ? <DataTable columns={["Metric", "Value", "Source"]} rows={metrics.filter((metric) => ["ga4", "google_search_console", "google_business_profile"].includes(String(metric.source))).slice(-12).map((metric) => [String(metric.metric_key ?? metric.metricKey), String(Number(numeric(metric.metric_value ?? metric.metricValue)).toLocaleString()), String(metric.source)])} /> : <p className="text-muted">No real Google metrics have been imported yet. Connect Google, configure IDs, then run a read-only sync.</p>}</ChartCard>
        <ChartCard title="GA4 top pages">{topPages.length ? <DataTable columns={["Page", "Views", "Measured"]} rows={topPages} /> : <p className="text-muted">No GA4 page metrics imported.</p>}</ChartCard>
        <ChartCard title="Search Console top queries">{topQueries.length ? <DataTable columns={["Query", "Clicks", "Impressions", "Position"]} rows={topQueries} /> : <p className="text-muted">No Search Console query metrics imported.</p>}</ChartCard>
