...
export default async function AiImprovementsPage() {
  ...
  return <>
    ...
    <section className="surface-card">
      ...
      <form className="form-grid" action="/api/studio/ai-employees/improvements" method="post">
    ...
    <section style={{ marginTop: 18 }}>
      {suggestions.length ? <div className="layout-grid">{suggestions.map((suggestion: any) => <ImprovementSuggestionCard key={suggestion.id} title={suggestion.title} status={String(suggestion.status)}>
        <p>{suggestion.summary}</p>
        <DataTable columns={["Workflow", "Decision", "Open"]} rows={[[suggestion.affected_workflow ?? suggestion.affectedWorkflow, <ApprovalBadge key="status" status={String(suggestion.owner_decision ?? suggestion.ownerDecision ?? "pending")} />, <a key="open" className="btn btn-secondary" href={`/studio/ai-employees/improvements/${suggestion.id}`}>Review</a>]]} />
