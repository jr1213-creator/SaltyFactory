...
export default async function CapabilityRequestsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      ...
      <form className="sf-form-grid" action="/api/studio/ai-employees/capability-requests" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Create Capability Request</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{requests.length ? <div className="sf-grid">{requests.map((request: any) => <CapabilityRequestPanel key={request.id} title={request.capability_name ?? request.capabilityName} status={String(request.status)}>
      <DataTable columns={["Employee", "Permission", "Risk", "Actions"]} rows={[[request.employee_id ?? request.employeeId, request.requested_permission_level ?? request.requestedPermissionLevel, request.risk_level ?? request.riskLevel, <span key="actions" className="sf-action-bar"><form action={`/api/studio/ai-employees/capability-requests/${request.id}/approve`} method="post"><button className="sf-button sf-button-secondary">Approve</button></form><form action={`/api/studio/ai-employees/capability-requests/${request.id}/grant`} method="post"><button className="sf-button sf-button-primary">Grant Safe Scope</button></form></span>]]} />
