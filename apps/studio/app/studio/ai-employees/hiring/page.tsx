...
export default async function AiHiringDeskPage() {
  ...
  return <>
    ...
    <section className="surface-card">
      ...
      <form className="form-grid" action="/api/studio/ai-employees/hiring/propose" method="post">
        ...
        <button className="btn btn-primary" type="submit">Create Hire Request</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>
      {requests.length ? <div className="layout-grid">{requests.map((request: any) => {
        ...
        return <HiringRequestCard key={request.id} title={request.requested_role_title ?? request.requestedRoleTitle} status={String(request.status)}>
          ...
          <DataTable columns={["Mission", "Forbidden actions", "Open"]} rows={[[spec?.mission ?? "Role spec pending", (spec?.forbidden_actions ?? spec?.forbiddenActions ?? []).slice(0, 8).join(", "), <a className="btn btn-secondary" href={`/studio/ai-employees/hiring/${request.id}`} key="open">Review</a>]]} />
        </HiringRequestCard>;
