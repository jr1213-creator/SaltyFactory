...
export default async function ToolAccessRequestsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      ...
      <form className="sf-form-grid" action="/api/studio/ai-employees/tool-access-requests" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Create Tool Access Request</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{requests.length ? <div className="sf-grid">{requests.map((request: any) => <ToolAccessPanel key={request.id} title={request.tool_name ?? request.toolName} status={String(request.status)}>
      <DataTable columns={["Employee", "Provider", "Access", "Actions"]} rows={[[request.employee_id ?? request.employeeId, request.provider_name ?? "internal", request.requested_access_level ?? request.requestedAccessLevel, <span key="actions" className="sf-action-bar"><form action={`/api/studio/ai-employees/tool-access-requests/${request.id}/approve`} method="post"><button className="sf-button sf-button-secondary">Approve</button></form><form action={`/api/studio/ai-employees/tool-access-requests/${request.id}/grant`} method="post"><button className="sf-button sf-button-primary">Grant Safe Scope</button></form></span>]]} />
