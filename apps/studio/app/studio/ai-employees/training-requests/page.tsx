...
export default async function TrainingRequestsPage() {
  ...
  return <>
    ...
    <section className="sf-card">
      ...
      <form className="sf-form-grid" action="/api/studio/ai-employees/training-requests" method="post">
        ...
        <button className="sf-button sf-button-primary" type="submit">Create Training Request</button>
      </form>
    </section>
    <section style={{ marginTop: 18 }}>{requests.length ? <div className="sf-grid">{requests.map((request: any) => <TrainingRequestPanel key={request.id} title={request.training_topic ?? request.trainingTopic} status={String(request.status)}>
      <DataTable columns={["Employee", "Gap", "Actions"]} rows={[[request.employee_id ?? request.employeeId, request.current_gap ?? request.currentGap, <span key="actions" className="sf-action-bar"><form action={`/api/studio/ai-employees/training-requests/${request.id}/approve`} method="post"><button className="sf-button sf-button-secondary">Approve</button></form><form action={`/api/studio/ai-employees/training-requests/${request.id}/complete`} method="post"><button className="sf-button sf-button-primary">Complete</button></form></span>]]} />
