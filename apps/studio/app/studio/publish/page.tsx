...
export default async function Page() {
  ...
  return <>
    ...
    <div className="sf-split-pane" style={{ marginTop: 18 }}>
      ...
      <Card><h2>Review Detail</h2><ProductArt label="Product Summary" /><ApprovalGateList gates={gates} /><RecommendationCard title="Automation cannot publish without human approval" description={gateResult.allowed ? "Gates are ready for guarded internal approval." : `Blocked: ${gateResult.blockedReasons.join(", ")}`} /><div className="sf-action-bar"><button className="sf-button sf-button-primary" disabled title="Use the repository-backed review workflow; public projection and provider sync remain separate guarded steps.">Use Review Workflow Below</button><button className="sf-button sf-button-secondary" disabled title="Request changes through the repository-backed review workflow.">Request Changes</button><button className="sf-button sf-button-danger" disabled title="Reject through the repository-backed review workflow.">Reject</button></div><h2>Audit trail</h2><AuditTimeline events={[{ title: "Review opened", detail: "Human review required before publish", time: "Current session" }, { title: "Gate evaluator", detail: gateResult.allowed ? "All gates pass" : "One or more gates failed", time: "Server-side" }]} /></Card>
    </div>
  </>;
}
