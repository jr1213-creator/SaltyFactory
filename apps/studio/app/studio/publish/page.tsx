...
export default async function Page() {
  const { publishReviews, drafts } = await getStudioLists();
  const review = publishReviews[0] as any ?? fixtures.publishReviewBlocked;
  const gateResult = evaluatePublishReviewGates(review);
  const gates = Object.entries(review.gates ?? fixtures.publishReviewBlocked.gates).map(([label, passed]) => ({ label: label.replaceAll("_", " "), passed: Boolean(passed), detail: passed ? "Passed" : "Blocks Shopify and Printify" }));
  return <>
