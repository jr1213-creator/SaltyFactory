...
export default async function PodBatchesPage() {
  ...
  return <>
    ...
    >
      <a className="btn btn-primary" href="/studio/pod-batches/new">New batch</a>
    </PageHeader>
    ...
    <section className="surface-card" style={{ marginTop: 18 }}>
      ...
      {batches.length ? <DataTable columns={["Batch", "Status", "Items", "Progress", "Open"]} rows={batches.map((batch: any) => {
        ...
        return [
          ...
          <a key={`${batch.id}-open`} className="btn btn-secondary" href={`/studio/pod-batches/${batch.id}`}>Open</a>
        ];
      })} /> : <EmptyState title="No batches yet" description="Create a batch to persist product draft work items for a provider-gated drop." action={<a className="btn btn-primary" href="/studio/pod-batches/new">Create batch</a>} />}
    </section>
  </>;
}
