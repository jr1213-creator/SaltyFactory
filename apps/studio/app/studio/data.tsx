...
export async function getStudioLists() {
  ...
  try {
    const repos = createRepositories();
    const [trends, clusters, phrases, briefs, jobs, assets, mockups, drafts, marginChecks, publishReviews, products, printifyProducts, productBatches, productBatchItems, providerConnections, integrationSyncRuns, workspaceMetrics, businessProfiles, channels, migrationGuides, baselines, podCandidates, dropshipCandidates, listingDraftsV1, socialContent, aiEmployees, aiEmployeeRuns, aiEmployeeOutputs, activity] = await Promise.all([
      ...
      repos.printify.listByWorkspace(studioWorkspaceId),
      repos.productBatch.listByWorkspace(studioWorkspaceId),
      repos.productBatchItem.listByWorkspace(studioWorkspaceId),
      ...
      repos.aiEmployee.runs.listByWorkspace(studioWorkspaceId),
