...
export default function Page() {
  ...
  return <>
    ...
    <div className="sf-grid sf-grid-4">
      ...
      <AiReadinessScoreCard title="SEO Score" score={64} />
      <AiReadinessScoreCard title="AEO Score" score={58} />
      <AiReadinessScoreCard title="GEO Score" score={54} />
    </div>
    <section className="sf-card" style={{ marginTop: 18 }}>
      <h2>Connected Integrations</h2>
      <div className="sf-grid sf-grid-4">{integrations.map((item) => <IntegrationCard key={item.key} title={item.label} status={item.status.replace(/_/g, " ")} tone={item.status === "configured" ? "success" : item.status === "disabled" ? "warning" : "danger"} />)}</div>
    </section>
