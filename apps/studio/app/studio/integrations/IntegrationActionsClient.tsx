...
export function IntegrationActionsClient({ integrations }: { integrations: Integration[] }) {
  ...
  return <section className="surface-card" style={{ marginTop: 18 }}>
    ...
    <div className="surface-card" style={{ display: "grid", gap: 12, marginBottom: 16 }}>
      <div>
        <strong>Google data sources</strong>
        <p className="text-muted">{google?.status.replace(/_/g, " ") ?? "not configured"} - {google?.googleEmail ? `Connected as ${google.googleEmail}` : "No Google email verified yet"}</p>
        <p className="text-muted">Scopes: {(google?.scopes ?? []).join(", ") || "Not connected"}</p>
      </div>
      <div className="action-bar">
        <button className="btn btn-primary" disabled={busyProvider !== ""} onClick={() => googleAction("connect")}>Connect Google</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected} title={googleConnected ? "Disconnect stored Google OAuth credential." : googleDisabledReason} onClick={() => googleAction("disconnect")}>Disconnect Google</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected} title={googleConnected ? "Run a live userinfo verification call." : googleDisabledReason} onClick={() => googleAction("test")}>Test Connection</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected} title={googleConnected ? "Discover GA4, Search Console, and Business Profile resources available to this Google account." : googleDisabledReason} onClick={() => googleAction("autoDetect")}>Auto-detect Google setup</button>
      </div>
      <div className="surface-card" style={{ display: "grid", gap: 10 }}>
        <strong>Google setup flow</strong>
        <p className="text-muted">1. Connect Google OAuth. 2. Auto-detect available Google resources. 3. Select or confirm GA4 and Search Console. 4. Sync data. Google Business Profile is optional for online-only Salty Cowhide POD launch readiness.</p>
        {detected ? <div className="layout-grid layout-grid-3">
          <div><strong>GA4</strong><p className="text-muted">{detected.dataSources.ga4.message}</p></div>
          <div><strong>Search Console</strong><p className="text-muted">{detected.dataSources.searchConsole.message}</p></div>
          <div><strong>Google Business Profile</strong><p className="text-muted">{detected.dataSources.businessProfile.message}</p></div>
        </div> : null}
      </div>
      <section className="surface-card" style={{ display: "grid", gap: 12 }}>
        <div>
          <strong>Create Google setup for SaltyCowhide.com</strong>
          <p className="text-muted">Use this when auto-detect finds other Google projects but no matching SaltyCowhide.com resources. Nothing is marked connected until a live sync or verification succeeds.</p>
        </div>
        <div className="layout-grid layout-grid-4">
          <div className="surface-card" style={{ display: "grid", gap: 8 }}>
            <strong>Create GA4 setup</strong>
            <p className="text-muted">Recommended. Creates a GA4 property and web stream for https://saltycowhide.com/ where Google Analytics edit permission allows it. Owner terms or account selection may be required.</p>
            <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected} title={googleConnected ? "Create GA4 setup for SaltyCowhide.com." : googleDisabledReason} onClick={() => googleSetupAction("ga4_create")}>Create GA4 setup for SaltyCowhide.com</button>
            <p className="text-muted">Manual fallback: create a GA4 property in Google Analytics, add a web stream for https://saltycowhide.com/, then save the numeric property ID.</p>
          </div>
          <div className="surface-card" style={{ display: "grid", gap: 8 }}>
            <strong>Add Search Console property</strong>
            <p className="text-muted">Recommended. Adds https://saltycowhide.com/ and sc-domain:saltycowhide.com where Search Console write permission allows it. Ownership verification is still required.</p>
            <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected} title={googleConnected ? "Add SaltyCowhide.com to Search Console." : googleDisabledReason} onClick={() => googleSetupAction("search_console_add")}>Add SaltyCowhide.com to Search Console</button>
            <p className="text-muted">Manual fallback: verify with DNS TXT, HTML file, meta tag, or Google Analytics verification if available.</p>
          </div>
          <div className="surface-card" style={{ display: "grid", gap: 8 }}>
            <strong>Set up Merchant Center</strong>
            <p className="text-muted">Important for ecommerce. Detects Merchant Center access and tracks business info, website claim, shipping, tax, product feed, and policy actions. It does not submit product feeds.</p>
            <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected} title={googleConnected ? "Set up Merchant Center for SaltyCowhide.com." : googleDisabledReason} onClick={() => googleSetupAction("merchant_center_setup")}>Set up Merchant Center for SaltyCowhide.com</button>
            <p className="text-muted">Manual fallback: create Merchant Center, claim SaltyCowhide.com, complete shipping/tax, then return for feed readiness.</p>
          </div>
          <div className="surface-card" style={{ display: "grid", gap: 8 }}>
            <strong>Google Business Profile eligibility check</strong>
            <p className="text-muted">Optional for online-only POD. Only proceed if Salty Cowhide has an eligible local presence.</p>
            ...
            <button className="btn btn-secondary" disabled={busyProvider !== ""} onClick={() => googleSetupAction("gbp_eligibility")}>Check Business Profile eligibility</button>
          </div>
        </div>
      </section>
      {ga4Candidates.length || gscCandidates.length || gbpCandidates.length ? <div className="form-grid">
      ...
      {warning ? <p className="alert-panel">{warning}</p> : null}
      ...
      <div className="action-bar">
        <button className="btn btn-secondary" disabled={busyProvider !== ""} onClick={() => setManualSetupOpen((value) => !value)}>Manual setup</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected || (!ga4PropertyId && !siteUrl && (!gbpAccountId || !gbpLocationId))} title={!googleConnected ? googleDisabledReason : "Save selected Google resources as configured, then run sync to verify."} onClick={() => googleAction("configure")}>Save selected Google resources</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected || !ga4PropertyId} title={!googleConnected ? googleDisabledReason : !ga4PropertyId ? "Configure GA4 property ID first." : "Import GA4 read-only metrics."} onClick={() => googleAction("ga4")}>Sync GA4</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected || !siteUrl} title={!googleConnected ? googleDisabledReason : !siteUrl ? "Configure Search Console site URL first." : "Import Search Console read-only search performance."} onClick={() => googleAction("gsc")}>Sync Search Console</button>
        <button className="btn btn-secondary" disabled={busyProvider !== "" || !googleConnected || !gbpAccountId || !gbpLocationId} title={!googleConnected ? googleDisabledReason : !gbpAccountId || !gbpLocationId ? "Configure GBP account and location first." : "Import GBP read-only account, location, and review summary."} onClick={() => googleAction("gbp")}>Sync Business Profile</button>
      </div>
      {(google?.setupRequired ?? []).length ? <p className="text-muted">Google setup required: {google?.setupRequired.join(", ")}</p> : null}
    </div>
    <div className="stack-list">
      {integrations.map((item) => <div key={item.key} className="surface-card" style={{ display: "grid", gap: 10 }}>
        <div><strong>{item.label}</strong><p className="text-muted">{item.status.replace(/_/g, " ")} - {item.capabilities.join(", ") || "No live capabilities configured"}</p></div>
        <div className="action-bar">
          <button className="btn btn-secondary" disabled={busyProvider !== ""} onClick={() => call(item.key, "status")}>Status</button>
          <button className="btn btn-secondary" disabled={busyProvider !== "" || ["google_oauth", "ga4", "google_search_console", "google_business_profile"].includes(item.key)} title={["google_oauth", "ga4", "google_search_console", "google_business_profile"].includes(item.key) ? "Use the Google test and sync buttons above." : undefined} onClick={() => call(item.key, "test")}>Test</button>
          <button className="btn btn-secondary" disabled={busyProvider !== "" || !["connected"].includes(item.status) || ["google_oauth", "ga4", "google_search_console", "google_business_profile"].includes(item.key)} title={["google_oauth", "ga4", "google_search_console", "google_business_profile"].includes(item.key) ? "Use the Google data source sync buttons above." : undefined} onClick={() => call(item.key, "sync")}>Sync Now</button>
        </div>
        {item.setupRequired.length ? <p className="text-muted">Setup required: {item.setupRequired.join(", ")}</p> : null}
        {item.lastSuccessfulSync ? <p className="text-muted">Last sync: {item.lastSuccessfulSync}</p> : null}
      </div>)}
    </div>
