...
export async function GET(req: Request) {
  try {
    ...
    const rows = await createRepositories().commerceAgent.qualityChecks.listByWorkspace(shopManagerWorkspaceId);
    return NextResponse.json({ ok: true, qualityChecks: rows });
  } catch (error) {
