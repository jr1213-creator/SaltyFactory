...
export async function POST(req: Request) {
  try {
    ...
    if (!gates.shopifyAllowed) blockingReasons.unshift("Shopify publish gates have not passed.");
    return NextResponse.json(
      {
        ok: false,
        status: "not_implemented",
        route: "publish/shopify",
        message: "Shopify draft export requires persisted publish review lookup and provider connection setup before it can run.",
        blockingReasons,
        gates,
        auditActor: actor
      },
