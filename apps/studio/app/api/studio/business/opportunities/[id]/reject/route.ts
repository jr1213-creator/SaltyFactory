...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return withBusinessApproval(req, async (repos, user) => {
      ...
      const updated = await repos.business.opportunities.update(id, { owner_decision: "rejected", updated_by: user.id } as any);
      return NextResponse.json({ ok: true, status: "rejected", opportunity: updated });
    });
  } catch (error) {
