...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return withBusinessApproval(req, async (repos, user) => {
      ...
      const updated = await repos.business.authorityRequests.update(id, { status: "expired", updated_by: user.id } as any);
      return NextResponse.json({ ok: true, status: "expired", authorityRequest: updated });
    });
  } catch (error) {
