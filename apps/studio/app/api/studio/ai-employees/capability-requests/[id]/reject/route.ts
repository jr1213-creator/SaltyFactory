...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await repos.aiWorkforce.capabilityRequests.update(id, { status: "rejected", updated_by: user.id } as any);
    return NextResponse.json({ ok: true, status: "rejected", request: updated });
  } catch (error) {
