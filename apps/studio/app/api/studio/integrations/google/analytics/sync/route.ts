...
import { syncGoogleAnalytics } from "@saltyfactory/integrations";
...
export async function POST(req: Request) {
  try {
    ...
    const result = await syncGoogleAnalytics({ repos: createRepositories(), workspaceId, actorId: user.id, config: parseEnv() });
    ...
    try {
      ...
      return NextResponse.json({ ok: false, status: "error", provider: "ga4", message: sanitizeProviderError(error) }, { status: 502 });
    }
  }
}
