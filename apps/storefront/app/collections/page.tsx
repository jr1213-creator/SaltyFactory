...
export default async function Page() {
  ...
  return <main><section className="sf-page-hero"><h1>Collections</h1><p>Coastal western capsules built from approved public product projections only.</p></section><section className="sf-section">{products.length ? <ProductGrid>{products.map((product:any)=><ProductCard key={product.id} title={product.title} price={product.price ?? "Review required"} badge="Approved" />)}</ProductGrid> : <div className="sf-empty"><strong>No approved public collections yet</strong><p>Collections appear only after products pass approval, publish gates, and public projection setup.</p></div>}</section></main>;
}
