...
export default async function Home() {
  ...
  return <main>
    ...
    <section className="sf-trust-row">{["Approved Public Catalog","Made to Order When Configured","Checkout Requires Provider Setup","Private Studio Data Hidden","Human-Reviewed Listings"].map((item) => <div key={item}><strong>{item}</strong><span>Public storefront content stays projection-safe.</span></div>)}</section>
    <section className="sf-section"><div className="sf-section-header"><h2>New Arrivals</h2><a href="/collections">View all</a></div>{display.length ? <ProductGrid>{display.map((product: any) => <a key={product.id ?? product.title} href={`/products/${product.handle ?? "demo-product"}`}><ProductCard title={product.title} price={product.price ?? "Review required"} description={product.description} badge={products.length ? "Approved" : "Development preview"} /></a>)}</ProductGrid> : <div className="sf-empty"><strong>No approved public products yet</strong><p>Products appear here only after approval, publish gates, and public projection setup.</p></div>}</section>
    <section className="sf-commerce-grid sf-section"><div><h2>Featured Collections</h2><div className="sf-empty"><strong>Collections pending approved products</strong><p>Collection cards publish only after approved public product projections exist.</p></div></div><div><h2>Fresh Drops & Stories</h2><div className="sf-story-grid"><article className="sf-story"><h3>Built for the Coast & Beyond</h3><p>Editorial content hub prepared for search-safe product stories.</p></article><article className="sf-story"><h3>Ranch Roots Collection</h3><p>Heritage-inspired content appears after approval.</p></article><article className="sf-story"><h3>Summer Essentials</h3><p>Public collection copy stays separate from private Studio data.</p></article><article className="sf-story"><h3>AI-Ready Content Hub</h3><p>Schema-ready content remains public-only and does not expose factory data.</p></article></div></div><aside><h2>Top POD Products</h2><p className="muted">Live rankings require Shopify analytics. Demo rankings are not shown as revenue.</p></aside></section>
  </main>;
}
