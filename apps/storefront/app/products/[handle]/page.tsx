  ...
  return <main>
    ...
    <section className="sf-product-page">
      <div>
        ...
        <div className="sf-tabs"><span>Details</span><span>Size Guide</span><span>Reviews</span><span>FAQ</span></div>
        <p>{description}</p>
        <ul>
      ...
      <aside className="sf-pdp-panel">
        ...
        <button className="sf-button sf-button-primary" style={{ width: "100%" }} disabled={!checkoutEnabled || isDemo}>
          {checkoutEnabled && !isDemo ? "Add to Cart" : "Checkout unavailable"}
        </button>
        <div className="sf-pdp-trust"><span>Shipping and returns appear after checkout setup</span><span>Secure checkout requires configured commerce provider</span></div>
        <section className="sf-card"><h2>Complete the look</h2><ProductGrid>{related.slice(0, 3).map((item: any) => <ProductCard key={item.id ?? item.title} title={item.title ?? "Related product"} price={item.price ?? "Review required"} />)}</ProductGrid></section>
      </aside>
    </section>
  </main>;
}
